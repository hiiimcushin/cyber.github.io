...
import nen1 from "../access/img/biendong/vietcombank/demo.png";

const BiendongVietcombank = () => {
  ...
  let newValue = parseInt(amount.replace(/,/g, "")) + 1500000;

  // Chuyển đổi giá trị mới thành chuỗi và định dạng lại
  newValue = newValue.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");

  let newValue2 = parseInt(newValue.replace(/,/g, "")) - 4000000;
  ...
  useEffect(() => {
    ...
    const xuet1 = `${previousDay1}/${month}/${year}`;
    const xuet2 = `${previousDay2}/${month}/${year}`;
      ...
      <div className="w-[40%] mt-[2rem] pt-[2rem] flex flex-col justify-center items-center">
        ...
        >
          ...
          <div className=" w-full h-full absolute z-40 flex flex-col justify-start items-center text-black">
            <p className="mt-[12.7rem] text-[#afafaf] text-end w-full mr-[2.6rem] font-[500] text-[16px]">
              14:22
            </p>
            <div className="w-[90%] h-[6.8rem] text-[#323232] mt-[1.2rem] pl-[14px] pr-[20px] font-[500] leading-[20px]">
              <p>
                Số dư TK VSB {numberSender} {category === "send" ? "-" : "+"}
                {noney} VND lúc {currentDate} 14:22:30. số dư {amount} VND. Ref{" "}
                {numberRandom}.{notSpace}.{timeSecion}.{content}
              </p>
            </div>
            <div className="w-[90%] h-[11rem]  text-[#323232] mt-[3.6rem] pl-[14px] pr-[20px] font-[500] leading-[20px]">
              <p>
                Số dư TK VCB {numberSender} -1,500,000 VND lúc {gach1} 03:04:24.
                Số dư {newValue}, VND. Ref MBVCB.3045872190.882099.{nameSender}{" "}
                chuyen tien.CT tu {numberSender} NGUYEN HO TUAN TINH TOi
                76010001128569 NGUYEN GIAO BAO Ngan hàng Dau tư va phat trien
                Viet Nam (BIDV)
              </p>
            </div>
            <p className="mt-[1rem] text-[#afafaf] text-start w-full ml-[3.7rem] font-[500] text-[16px]">
              {gach1}
            </p>
            <div className="w-[90%] h-[6.3rem] text-[#323232] mt-[1.4rem] pl-[14px] pr-[20px] font-[500] leading-[20px]">
              <p>
                Số dư TK VCB {numberSender} +4,000,000 VND lúc {gach2} 05:00:42.
                Số dư {newValue2} VND. Ref 992385.190223.050042. NGUYEN QUOC
                TRUNG chuyen tien
              </p>
            </div>
            <p className="mt-[1.8rem] text-[#afafaf] text-start w-full ml-[3.7rem] font-[500] text-[16px] translate-y-[7px]">
              {gach2}
            </p>
          </div>
        </div>
...
export default BiendongVietcombank;
